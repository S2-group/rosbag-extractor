...
graph_info = pd.DataFrame(data={'name': nodes}, columns=['name', 'msg', 'topics'])
list_of_topics = []
list_of_msgs = []
...
for i in range(len(nodes)):
    for j in range(len(all_info)):
        ...
        if all_info['name'][j] == nodes[i]:
            ...
            # TODO: merge msgs;
            #  -> ast_literal_eval is not working for '/'
            # merge the message contains the substring "Subscribing to " with the same node name
            # if substring in all_info['msg'][j]:
            #    list_of_msgs += all_info['msg'][i][15:]
